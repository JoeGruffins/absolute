...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(out.nValue == MASTERNODE_COLLATERAL * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %s ABS, masternode=%s\n",MASTERNODE_COLLATERAL, vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when MASTERNODE_COLLATERAL ABS tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for MASTERNODE_COLLATERAL ABS tx -> 1 confirmation
