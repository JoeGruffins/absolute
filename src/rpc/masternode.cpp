...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (
                strMode != "activeseconds" && strMode != "addr" && strMode != "daemon" && strMode != "full" && strMode != "info" && strMode != "details" &&
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. \"mode\"      (string, optional/required to use filter, defaults = details) The mode to run list in\n"
		...
		"  details        - Print info in JSON format (can be additionally filtered, partial match)\n"
                "                   (can be additionally filtered, partial match)\n"
                "  lastpaidblock  - Print the last block height a node was paid on the network\n"
                "  lastpaidtime   - Print the last time a node was paid on the network\n"
                "  lastseen       - Print timestamp of when a masternode was last seen on the network\n"
                "  payee          - Print Absolute address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
    ...
    if (strMode == "full" || strMode == "details" || strMode == "lastpaidtime" || strMode == "lastpaidblock") {
        CBlockIndex* pindex = NULL;
        {
            LOCK(cs_main);
            pindex = chainActive.Tip();
